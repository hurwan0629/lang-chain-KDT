...
class UserService(BaseService):
  ...
  # 특정 아이디가 존재하는지 확인
  @service_decorator()
  def check_username_duplicated(self, cur, id: str) -> bool:
    if self._dao.check_username_exists(cur, id) >= 1:
      return True
    return False
  ...
  def check_email_duplicated(self, cur, email: str) -> bool:
    if self._dao.check_email_exists(cur, email) >= 1:
      return True
    return False
  ...
  def regist_user(self, cur, id, password, name, email) -> Optional[UserRegistDTO | None]:
    return self._dao.insert_user(cur, id, password, name, email)

  # 로그인 시도하기. 성공하면 UserLoginSuccessDTO 반환
  @service_decorator()
  def try_login(self, cur, username, password) -> Optional[UserLoginSuccessDTO | None]:
    result: Optional[UserLoginSuccessDTO | None] = self._dao.login_check(cur, username, password) 
    return result
