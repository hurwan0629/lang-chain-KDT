...
자바스크립트(JavaScript)는 웹 브라우저에서 동작하는 프로그래밍 언어로, HTML과 CSS로 만들어진 정적인 웹 페이지에 동적인 기능과 사용자와의 상호작용을 추가하는 역할을 합니다. 버튼 클릭, 입력값 처리, 화면 내용 변경, 애니메이션 제어, 서버와의 데이터 통신 등 다양한 기능을 수행할 수 있으며, 오늘날에는 브라우저를 넘어 서버(Node.js), 모바일, 데스크톱 애플리케이션까지 활용 범위가 확장된 핵심 웹 기술입니다.
...
## 자바스크립트의 실행
1. 브라우저가 페이지를 여는 순간 
사용자가 URL을 입력하면 브라우저는 서버로부터 HTML문서를 받아옵니다.
이때 브라우저는 HtML을 위에서 아래로 한줄씩 읽기 시작합니다.

> 이 과정을 파싱이라고 합니다.

2. HTML 파싱 중 JavaScript를 만나면
HTML을 읽다가 <script> 태그를 만나면 아래와 같이 진행햅나디.
  - HTML 파싱 일시 중단
  - JavaScript 파일 다운로드
  - JavaScript 실행
  - 실행이 끝난 뒤 다시 HTML 파싱 재개

# 웹소켓
  통신 방식
1. HTTP 요청으로 웹소켓으로 업그레이드 해달라고 요청(handshake)
2. HTTP -> WebSocket 프로토콜로 변경
3. 연결이 유지된 상태에서 서버 또는 클라이언트가 푸시를 할 수 있음

Node.js + WebSocket의 장점
1. 동시접속자 많은 시스템에 유리
   - 채팅서버, 게임서버, 실시간 모니터링 등 많이 사용
2. JavaScript 풀스택
   - 클라이언트(브라우저) JS, 서버도 JS 개발 모두 수월
   - WebSocket 메시지에 JSON을 사용하면 서버와 클라이언트 간 데이터 교환이 직관적